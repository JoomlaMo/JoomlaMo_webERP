...
If($mainframe->getUserState( "post")){
	...
	If(array_key_exists('second', $PostPrevious)){
		$this->item['Second'] = $PostPrevious['second'];
		$Second = $PostPrevious['second'];
	}
	If(array_key_exists('hour', $PostPrevious)){
		$this->item['Hour'] = $PostPrevious['hour'];
		$Hour = $PostPrevious['hour'];
	}
	If(array_key_exists('minute', $PostPrevious)){
		$this->item['Minute'] = $PostPrevious['minute'];
		$Minute = $PostPrevious['minute'];
	}
	$this->item['Time'] = $Hour . ":" . $Minute . ":" . $Second;
}
...
<script language="javascript" src="components/com_teams/mootools.v1.11.js"></script>
...
<div style="float:left"><h1><?php echo JText::_('GAME'); ?></h1></div>
<div class="div_block" style="float:right;"><?php echo teamseditHelperToolbar::getToolbar();?></div><br><br><br><br>
		...
		<tr>
			<td width="100" align="right" class="key">
				<label for="Time" <?php echo $TimeStyle  ?> >
					<abbr title="<?php echo JText::_('ENTER_TIME'); ?>"><?php echo JText::_( 'TIME(HH:MM:SS)' ); ?>:</abbr>
				</label>
			...
			<td  align=left  > 
				<select name=hour>
<?php
$Hour = substr($this->item['Time'],0,2);
for($i=0;$i<25;$i++) {
	If($i == $Hour){
		$Selected = 'SELECTED';
	}else {
...
?>						
				...
				<select name=minute>
<?php
$Minute = substr($this->item['Time'],3,2);
for($i=0;$i<60;$i++) {
	If($i == $Minute){
		$Selected = 'SELECTED';
	}else {
...
?>						
				...
				<select name=second>
<?php
$Second = substr($this->item['Time'],6,2);
for($i=0;$i<60;$i++) {
	If($i == $Second){
		$Selected = 'SELECTED';
	}else {
		...
		<tr>
			<td width="100" align="right" class="key">
				<label for="Description" <?php echo $DescriptionStyle  ?> >
					<abbr title=<?php echo JText::_( "ENTER_LINE_ABOUT_GAME"); ?>><?php echo JText::_( 'DESCRIPTION' ); ?>:</abbr>
				</label>
			</td>
			<td colspan="2">
...
?>
	...
	<input type="hidden" name="cid[]" value="<?php echo $this->item['GameID']; ?>" />
