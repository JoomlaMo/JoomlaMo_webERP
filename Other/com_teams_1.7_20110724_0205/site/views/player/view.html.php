...
class teamsViewplayer extends JView
{
    function display($tpl = null)
    {
      ...
      JHTML::stylesheet( 'team.css', 'administrator/components/com_team/assets/' );
		...
		$document->setTitle( JText::_('Player') );
      //-- Get data from the model
      $item =& $this->get('Data');
      $teams  				= & teamsHelper::getTeamNames();
      $uniforms  			= & teamsHelper::getUniformDescriptions();
      $TeamPlayer 		= & teamsHelper::getTeamPlayer($item['PlayerID']);
      $uniformnumbers 	= & teamsHelper::getUniformNumbers($item['PlayerID']);
      $isNew 				= ($item['PlayerID'] < 1);
      $text 				= $isNew ? JText::_('New') : JText::_('Edit');


      $this->assignRef('item', 				$item);
      $this->assignRef('uniforms', 			$uniforms);
      $this->assignRef('uniformnumbers', 	$uniformnumbers);
