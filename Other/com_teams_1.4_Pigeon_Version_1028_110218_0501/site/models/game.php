...
class teamsModelgame extends JModel
{
    ...
    {    	
    	...
    	If($data['GameID'] > 0){
			...
			$query = "INSERT INTO #__team_games ";
			$where = '';
		}	
    ...
    function delete()
    {
      ...
      if (count( $cids )) {
         $query = 'DELETE FROM #__team_games WHERE GameID IN(' . $cidstring . ')';
         $db->setQuery($query);
			If(!$result = $db->query()){
				return false;
			}
