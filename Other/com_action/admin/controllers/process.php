...
class ActionsControllerprocess extends JController
{
	var $cid;
	...
	{
		$this->_query = 'UPDATE #__action_process'
	...
	function cancel()
	{
		...
		$this->setRedirect( 'index.php?option=com_Action&view=processs', $msg );
	}
	function publish()
	{
		...
		$link = 'index.php?option=com_Action&view=processs';
	...
	function save()
	{
		...
		$post['EstimatedTime'] = "0" . $post['Hours'] . ":0" . $post['Minutes'] . ":0" . $post['Seconds'];
	...
	function _reOrder($direction)
	{
		...
		if (isset( $cid[0] ))
		{
			$row = & JTable::getInstance('process', 'Table');
			$row->load( (int) $cid[0] );
			$row->move($direction);
		...
		$mainframe->redirect('index.php?option=com_Action&view=processs');
	}
	...
	function saveorder()
	{
		...
		$mainframe->redirect('index.php?option=com_Action&view=processs', $msg);
	}
